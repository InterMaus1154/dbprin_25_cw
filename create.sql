...
-- For membership expiry checks and queries filtering by active memberships
CREATE INDEX idx_customer_membership_dates ON customers(mship_id, mship_end_date)
WHERE
    mship_id IS NOT NULL;

-- For finding customers whose memberships are about to expire
CREATE INDEX idx_customer_membership_expiry ON customers(mship_end_date)
WHERE
    mship_id IS NOT NULL
    AND mship_end_date IS NOT NULL;
...
-- For finding who's working on a specific day
CREATE INDEX idx_staff_schedule_day ON staff_schedule(schedule_day, staff_id);

-- For checking a specific staff member's schedule
CREATE INDEX idx_staff_schedule_staff_day ON staff_schedule(staff_id, schedule_day);
...
-- For branch managers viewing bookings by date at their branch
CREATE INDEX idx_booking_branch_date ON bookings(branch_id, booking_date, booking_time);

-- Prevent double-booking the same vehicle at the same time (UNIQUE constraint)
CREATE UNIQUE INDEX idx_unique_vehicle_booking ON bookings(vec_id, booking_date, booking_time);
...
-- For quick lookup by invoice number (user-facing searches)
CREATE INDEX idx_invoice_number ON invoices(inv_number);

-- For finding overdue invoices by branch
CREATE INDEX idx_invoice_status_due ON invoices(inv_status, inv_due_date);
...
-- For finding all overdue installments across invoices
CREATE INDEX idx_installment_due_date_status ON installments(inst_due_date, inst_status);

-- For invoice-specific installment queries
CREATE INDEX idx_installment_invoice_status ON installments(inv_id, inst_status, inst_due_date);
...
-- For finding all jobs associated with a specific booking service
CREATE INDEX idx_job_booking_service ON jobs(booking_service_id, job_status);

-- For finding jobs assigned to a bay (bay management)
CREATE INDEX idx_job_bay_status ON jobs(bay_id, job_status);
...
-- For finding low stock items at a branch
CREATE INDEX idx_branch_parts_quantity ON branch_parts(branch_id, quantity);

-- For checking stock of a specific part across branches
CREATE INDEX idx_branch_parts_part_stock ON branch_parts(part_id, quantity);
...
-- For finding all parts used in a specific job
CREATE INDEX idx_part_usage_job ON part_usage(job_id);

-- For tracking usage of specific parts across jobs
CREATE INDEX idx_part_usage_part ON part_usage(part_id, quantity);
...
UPDATE
,
    DELETE ON bookings TO technician;
...
UPDATE
,
    DELETE ON booking_packages TO technician;
...
UPDATE
,
    DELETE ON booking_services TO technician;
...
,
INSERT
    ON part_transfers TO technician;
...
GRANT
SELECT
    ON staff_certifications TO technician;
