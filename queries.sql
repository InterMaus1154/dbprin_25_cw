...
SELECT
    RANK() OVER (
        ORDER BY
            SUM(
                COALESCE(id.paid_inv_final, 0) + COALESCE(id.due_inv_final, 0)
            ) DESC
    ) AS "Branch Total Income Rank",
    ...
    CONCAT(
        COALESCE(
            ROUND(
                SUM(id.due_count) * 100.00 / NULLIF(SUM(id.total_invoices), 0),
                2
            ),
            0
        ),
    ...
    COALESCE(
        MAX(CONCAT_WS(' ', s.staff_fname, s.staff_lname)),
        'N/A'
    ) AS "Most Jobs Completed By",
...
SELECT
    CONCAT_WS(': ', part_id, part_name) AS "Part",
            ...
            WHERE
                EXISTS (
                    SELECT
                        1
                    FROM
                        customer_feedback_analyst cfa
                    WHERE
                        cfa.booking_id = b.booking_id
                )
        ) as count
    FROM
        bookings b
...
SELECT
    (
        SELECT
            count
        FROM
            booking_count_with_feedbacks
    ) AS "Bookigns with feedbacks",
    CONCAT(
        ROUND(
            (
                (
